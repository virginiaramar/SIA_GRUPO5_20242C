...
def enhanced_pygame_visualize(ga):
    pygame.init()
    WIDTH, HEIGHT = 1200, 700  # Tamaño más compacto
    ...
    class CharacterSprite:
        def __init__(self, character, x, y, index):
            ...
            self.size = max(5, min(15, int(5 + character.get_performance() * 2)))
            self.index = index
            self.selected = False
    ...
    def draw_fitness_graph(history):
        ...
        screen.blit(surf, (WIDTH - size[0] - 10, 10))
        plt.close()

    population_sprites = create_sprite_grid(ga.initialize_population(), 20, 60, 20)
    generation = 0
    history = []
    ...
    while running and generation < ga.stop_criteria['max_generations']:
        ...
        # Título y generación actual
        draw_text(f"Generación {generation}", 20, 20, font_large)

        # Población actual
        draw_text("Población:", 20, 60, font_medium)
        ...
        for parent in parents:
            for i, sprite in enumerate(population_sprites):
                if sprite.character == parent:
                    ...
                    break
        draw_text(f"Selección: {len(parents)} padres", 20, HEIGHT - 120, font_medium)
        draw_text(f"Índices: {', '.join(map(str, selected_indices[:10]))}{'...' if len(selected_indices) > 10 else ''}", 20, HEIGHT - 90, font_small)
        ...
        offspring_sprites = create_sprite_grid(offspring, WIDTH - 250, 60, 10)
        draw_text(f"Descendencia: {len(offspring)}", WIDTH - 250, 30, font_medium)
        for sprite in offspring_sprites:
            sprite.draw(screen)
        ...
        draw_text(description, 20, HEIGHT - 60, font_small)

        population_sprites = create_sprite_grid(new_population, 20, 60, 20)
        ...
        pygame.display.flip()
        clock.tick(2)  # 2 FPS para dar tiempo a observar, pero no demasiado lento
