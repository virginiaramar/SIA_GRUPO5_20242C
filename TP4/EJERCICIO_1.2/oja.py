...
# Algoritmo para la regla de Oja
...
def plot_eigenvector_values(w, feature_names, last_learning_rate, total_epochs, eigenvalue):
    ...
    for index, bar in enumerate(bars):
        ...
        if height > 0:
            plt.text(bar.get_x() + bar.get_width() / 2, height + 0.02, f'{height:.4f}', ha='center', va='bottom', fontsize=9, color='black')
        else:
            plt.text(bar.get_x() + bar.get_width() / 2, height - 0.02, f'{height:.4f}', ha='center', va='top', fontsize=9, color='black')
    info_text = f'Epochs: {total_epochs}\nLast LR: {last_learning_rate:.4f}\nEigenvalue: {eigenvalue:.4f}'
    plt.text(0.95, 0.05, info_text, transform=plt.gca().transAxes, fontsize=10, color='red', ha='right', va='bottom', bbox=dict(facecolor='white', alpha=0.7, edgecolor='gray'))
...
def plot_countries_pc1(countries, X_normalized, w_oja, last_learning_rate, total_epochs):
    ...
    for bar in bars:
        width = bar.get_width()
        if width > 0:
            plt.text(width + 0.05, bar.get_y() + bar.get_height() / 2, f'{width:.2f}', va='center', ha='left', color='black')
        elif width < 0:
            plt.text(width - 0.05, bar.get_y() + bar.get_height() / 2, f'{width:.2f}', va='center', ha='right', color='black')
    info_text = f'Epochs: {total_epochs}\nLast LR: {last_learning_rate:.4f}'
    plt.text(0.98, 0.03, info_text, transform=plt.gca().transAxes, fontsize=10, color='red', ha='right', va='bottom', bbox=dict(facecolor='white', alpha=0.7, edgecolor='gray'))
    plt.xlabel('PC1')
